...
- 战斗同步目标周期 150ms，扣除请求耗时，串行且失败退避；实际频率受网络限制。Canvas 使用最多 350ms 的显示预测和连续特效；不改服务端分数。
...
先评估在线同步延迟和房间负载，再建设服务端 Python 隔离执行与专用房间调度；之后增加持久回放、团队和 8 人模式、迷雾与多地图。同步流畅度优化与性能采样见 `docx/05-同步与画面流畅度优化.docx` 和 `scripts/performance-check.cjs`。多轮规则与本次优化验收详见 `docx/04-多轮赛制与优化计划.docx`，当前状态见 03；01、02 保留为首版历史设计。
